...
In this project there are two levels. Issue levels and Engagement levels. Issue level describes the level that a issue can have (Aufgabe). Engagement level describes the progress level that the actual created worker has. We are going to focus in this implementation on the Engagement level (In the ui it will refer to just the Woker that is created because that is the german wording for it)

High level of what this feature should do:

-The owner of the company can create their own Worker levels
-The owner can update those worker levels
-The owner can delete those worker levels
-Owner cannot delete all worker levels - there must always be one worker level to choose from
...
Fetch those 4 worker levels using this endpoint: GET /org/statuses?entityType=engagement
...
-Ensure with zod validation that the worker level has a name

Ensure that the above endpoints can only be authroized when the role is Owner. Not anyone else can make those updates.
Note above implementation is similiar to that of the status.md application
