...
const OnOf_Worker_Procedure = ({ id, search, }) => {
    ...
    const searchData = data.form.fields.filter((field) => field.description
        .toLowerCase()
    ...
    async function openEditModal(id, description, editcomment, select_option, form_field_id) {
        toggleModal();
        setModalState({
            selectedItem: {
                id,
    ...
    async function insertHistoryData(result, user) {
        try {
    ...
    return (<>
            <div className="flex flex-col fixed w-full max-w-5xl h-150 rounded-2xl mx-auto  shadow-gray-200 shadow-lg overflow-auto p-6">
                <div className="w-max flex justify gap-5">
                    <Input value={descriptionSearch} onChange={(e) => setDescriptionSearch(e.target.value)} placeholder="Search"/>
                    <Button variant={"outline"} className={activetab === "form" ? "active bg-gray-200" : ""} onClick={() => setActiveTab("form")}>
                        Prozess
                    </Button>
                    <Button variant={"outline"} className={activetab === "files" ? "active bg-gray-200" : ""} onClick={() => setActiveTab("files")}>
                        Dateien
                    </Button>
                </div>
                <div>
                    {activetab === "files" && <Worker_Backround id={id}/>}
                    {activetab === "form" && (<div className="">
                            {modalState.selectedItem && (<div className="fixed inset-0 z-50 flex">
                                    <div onClick={closeModal} className="fixed inset-0 bg-black/50 cursor-pointer" aria-label="Close modal"/>
                                    <PreviewComponent toggleModal={toggleModal} id={modalState.selectedItem.id} description={modalState.selectedItem.description} editcomment={modalState.selectedItem.editcomment} select_option={modalState.selectedItem
                    .select_option} form_field_id={modalState.selectedItem
                    .form_field_id} handleSubmit={handleSubmit}/>
                                </div>)}

                            {sortedData.map((field, index) => (<Form key={index} id_original={field.id} editcomment={field.edit} select_option={field.status} description={field.description} owner={field.owner} form_field_id={data.form.id} onEdit={(id, description, editcomment, select_option, form_field) => openEditModal(id, description, editcomment, select_option, form_field)} handleSubmit={handleSubmit}/>))}
                        </div>)}
                </div>
            </div>
        </>);
