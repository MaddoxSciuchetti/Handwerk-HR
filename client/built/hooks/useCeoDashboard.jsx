...
function useCeoDashboard() {
    ...
    const { data: allEmployeeData, isLoading, error, } = useQuery({
        queryKey: ["ceo-dashboard"],
        queryFn: fetchChefData,
    });
    const uniqueHandwerkerProBSBEmployee = useHandwerkerProBSBEmployee(allEmployeeData);
    console.log("unique users by owner");
    console.log(uniqueHandwerkerProBSBEmployee);
    const currentBSBEmployee = useMemo(() => allEmployeeData?.filter((item) => item.owner === selectedUser) ||
