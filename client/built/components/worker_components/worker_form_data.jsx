...
const Form = ({ id_original, description, owner, editcomment, select_option, form_field_id, handleSubmit, onEdit,
// historyResult,
 }) => {
    const [selectedValue, setSelectedValue] = useState(select_option || "");
    const [editcommentValue, setEditComment] = useState(editcomment || "");
    const { historyData, isLoading, error, refetchHistory } = useGetHistory(id_original);
    useEffect(() => {
    ...
    return (<>
      <div className="justify-center items-center hover:scale-101 mt-10">
        <form className="flex flex-col  " onSubmit={handleSubmit} name="valuesform">
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-row mt-2">
              ...
              <img className="" src="/public/assets/Edit React Icon.svg" alt="text" onClick={() => onEdit(id_original, description, editcomment, select_option, form_field_id)}/>
            </div>
            <div className="flex gap-2 ">
              <span className={owner === "Siemon"
            ...
            : owner === "Acosta"
              ...
              <div>
                <span className={selectedValue === "erledigt"
            ...
            : selectedValue === "offen"
                ...
                : selectedValue === "in_bearbeitung"
                  ...
                  {selectedValue === "erledigt" ? (<span>Erledigt</span>) : selectedValue === "in_bearbeitung" ? (<span>In Bearbeitung</span>) : selectedValue === "offen" ? (<span>Offen</span>) : (<span>Status</span>)}
                </span>
              </div>
              ...
              <div className="relative">
                <span className="rounded-2xl bg-gray-100 px-3 py-1 text-sm cursor-pointer group">
                  ...
                  <div className="absolute bottom-full left-0 mb-2 p-3 bg-white border rounded-lg shadow-lg opacity-0 group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-10 ">
                    {editcommentValue === "" ? (<span>Kein Kommentar</span>) : (editcommentValue)}
                  </div>
                </span>
              </div>
            </div>
            ...
            <Accordion type="single" collapsible className="max-w-6xl">
              <AccordionItem value="shipping" className="mb-10 ">
                ...
                <AccordionContent className="">
                  {isLoading ? (<p>Loading History</p>) : ((historyData || []).map((item, index) => (<div key={index} className="">
