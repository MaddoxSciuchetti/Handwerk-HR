import { Home, Inbox, HandMetal, } from "lucide-react";
...
export function AppSidebar({}) {
    ...
    const hasPermission = useMemo(() => {
        return (requiredPermission) => {
            if (!requiredPermission)
                return true;
            if (user?.user_permission !== requiredPermission)
                return false;
            return true;
        };
    ...
    return (<Sidebar className="bg-gray-100 rounded-2xl">
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu className="text-black">
              {accessibleItems.map((item, index) => (<SidebarMenuItem className="text-black" key={index}>
