...
export default function FileUpload01({ id, setModal }) {
    const fileInputRef = useRef(null);
    const [uploadedFiles, setUploadedFiles] = useState([]);
    const [fileProgresses, setFileProgresses] = useState({});
    const queryClient = useQueryClient();
    const { mutate: uploadfiles, data: FileResponse, isPending: isLoading, error, } = useMutation({
        mutationFn: (files) => postFile(files, id),
        onSuccess: () => {
      ...
      <Card className="max-h-min mt-40 mx-auto text-center items-center z-50 bg-gray-200 rounded-xl  w-2xl">
        <div className="flex items-center gap-10 justify-center mb-6 m-10">
          ...
          <CardContent className="">
            ...
            {/* <Form /> */}
            <FileDropzone fileInputRef={fileInputRef} handleBoxClick={handleBoxClick} handleDragOver={handleDragOver} handleDrop={handleDrop} handleFileSelect={handleFileSelect}/>
            <FileList uploadedFiles={uploadedFiles} fileProgresses={fileProgresses} removeFile={removeFile}/>
            <div className="px-6 py-3 border-t border-border bg-muted rounded-b-lg flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <Button variant={"outline"} onClick={handleFileSubmit} className="h-9 px-4 text-sm font-medium hover:text-black">
                  Erstellen
                </Button>
              </div>
            </div>
